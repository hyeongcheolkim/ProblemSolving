...
bool move_piece(int idx, bool flag = false)
{
    ...
    auto iter = find_if(board[row][col].begin(), board[row][col].end(), [&](pair<int, int>& e) {
        return e.first == idx;
    });
    vector<pair<int, int>> sub_vec{iter, board[row][col].end()};
    board[row][col].resize(iter - board[row][col].begin());
    const auto& [dr, dc] = Direct[sub_vec.front().second];
    int nr = row + dr;
    int nc = col + dc;
    switch (terrain[nr][nc])
    {
        ...
        case WHITE:
            board[nr][nc].insert(board[nr][nc].end(), sub_vec.begin(), sub_vec.end());
            for (const auto& [idx, ignore] : sub_vec)
                pos[idx] = {nr, nc};
        ...
        case BLUE:
            if (flag)
            {
                board[row][col].insert(board[row][col].end(), sub_vec.begin(), sub_vec.end());
                if (board[row][col].size() >= 4)
                    return true;
            }
            else
            {
                rotate_counter(sub_vec.front().second);
                board[row][col].insert(board[row][col].end(), sub_vec.begin(), sub_vec.end());
